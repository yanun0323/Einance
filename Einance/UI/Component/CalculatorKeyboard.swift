...
//    @State private var value: String = ""
    ...
    init(input: Binding<String>, action: @escaping () -> Void = {}) {
        ...
        let w = System.device.screen.width
        let h = System.device.screen.height / 2 - 15
...
//        .padding(unitGap)
//        .padding(.bottom, 15)
//        .frame(width: System.device.screen.width, height: System.device.screen.height/2)
...
//        .shadow(radius: 3)
    ...
    private func buttonBlueprint(_ color: Color = .orange, _ scaleW: CGFloat = 1, _ scaleH: CGFloat = 1, action: @escaping () -> Void, @ViewBuilder content: @escaping () -> some View) -> some View {
        ...
        ButtonCustom(width: width, height: height, color: color, radius: unitRadius, action: action, content: content)
    }
}
...
//            value = ""
        ...
        if f > 10 { f = 10 }
        input = String(format: "%.\(f)f\(needDot ? "." : "")", d.ToDouble())
