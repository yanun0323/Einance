...
struct StatisticPage: View {
    ...
    init(injecter: DIContainer, budget: Budget) {
        ...
        self._width = State(initialValue: System.screen.width/(buttonCount+1))
        self.amountSums = []
        self.costSums = []
        self.balanceSums = []
        self.showableData = 0
    ...
    private func piePanel(for t: FinanceCategory, radiusRatio: CGFloat = 0.08, lineWidthRatio: CGFloat = 0.04) -> some View {
        if let data = t.data(a: amountSums, c: costSums, b: balanceSums),
           let sum = data.last {
            let lineWidth: CGFloat = System.screen.width * lineWidthRatio
            let radius: CGFloat = System.screen.width * radiusRatio
            ...
            HStack {
                ...
                Spacer()
                let blockSize: CGFloat = System.screen.width * 0.02
                VStack(alignment: .leading, spacing: 0) {
                    ForEach(budget.book.indices, id: \.self) { i in
                        ...
                        if p != 0 && !c.isForever {
                            HStack {
                                Block(width: blockSize, height: blockSize, color: c.bColor)
                                Text(c.name)
                                    .foregroundColor(.primary25)
                                    .frame(width: (System.screen.width - radius) * 0.23, alignment: .leading)
                                Text("\(Int(((p/sum)*100).double))%")
                                    .frame(width: (System.screen.width - radius) * 0.16, alignment: .leading)
                            }
    ...
    private func pieSlice(for index: Int, before: Decimal, sum: Decimal, value: Decimal, radius: CGFloat, line: CGFloat, showText: Bool = false) -> some View {
        ...
        pieCircle(from: f.cgfloat, to: t.cgfloat - gap, color: budget.book[index].bColor, width: width, radius: radius, padding: line, only: only)
    ...
    private func rectChartView() -> some View {
        Chart {
            ...
            ForEach(budget.book) { card in
                ...
                .foregroundStyle(card.bColor.opacity(0.4))
                .annotation {
                    if !card.amount.isZero {
                        Text(card.amount.description)
                            .foregroundColor(card.bColor.opacity(0.4))
                            .padding(.bottom, 5)
                            .fontWeight(.bold)
                    }
                }
...
extension StatisticPage {
    private func textOffset(for index: Int, data: [Decimal], radius: CGFloat) -> CGSize {
        ...
        let angle = CGFloat(sliceOffset + 2 * .pi * dataRatio.double)
