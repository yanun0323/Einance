...
struct CardAnalysisPage: View {
    ...
    private let scrollOuter: CGFloat = System.screen.width - 40
    private let outer: CGFloat = System.screen.width - 40
    private let inner: CGFloat = System.screen.width - 50 - 50/* unitHeight */
...
//            dataList()
        ...
        .onAppeared {
            ...
            dayRatio = inner/(maxDayCost.cgfloat)
            monthRatio = inner/(maxMonthCost.cgfloat)
    ...
    private func typeSelectorButton(set t: ChartType, _ s: LocalizedStringKey, _ w: CGFloat, _ h: CGFloat) -> some View {
        Button(width: w, height: h) {
            ...
            Text(s)
                .foregroundColor(chartType == t ? card.fColor : .section)
        }
    }
                    ...
                    HStack(spacing: 0) {
                        ForEach(dict.keys, id: \.self) { date in
                            ...
                            ZStack {
                                ...
                                .offset(y: pinned - (set.cost.cgfloat*ratio))
    ...
    private func chartLine(_ dotSize: CGFloat) -> some View {
        ...
        Path() { p in
            for d in dict.keys.reversed() {
                let set = dict[d]!
                let point = CGPoint(x: xFixed + CGFloat(count)*gap, y: yFixed - set.cost.cgfloat*ratio)
                if first {
                    p.move(to: point)
                    first = false
                }
        ...
        .stroke(card.bColor, lineWidth: lineWidth)
        .frame(width: w)
    }
